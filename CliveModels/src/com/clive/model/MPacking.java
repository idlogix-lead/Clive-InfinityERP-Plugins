...
public class MPacking extends X_ER_Packing {
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		BigDecimal LastingQty = Env.ZERO;
		BigDecimal PackQty = Env.ZERO;

		StringBuilder sb = new StringBuilder("SELECT \n");

		sb.append("coalesce(sum(qty), 0) as totalpackqty,\n" + 
				"        (SELECT coalesce(sum(qty), 0) from er_lasting where m_production_id = " + MPID + ") as totlalastingqty\n" + 
		...
		try
		{
			...
			if (rs.next ())
			{
				LastingQty = rs.getBigDecimal("totlalastingqty");
		...
		if (LastingQty.compareTo(PackQty.add(getQty())) == -1)
		{
